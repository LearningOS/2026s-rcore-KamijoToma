...
impl MemorySet {
    ...
    ///
    /// `start` must be page-aligned, `len` must be non-zero, and the resulting range must stay
    /// below the trap context. `port` is interpreted as a `R/W/X` bitmask and must be non-zero.
    /// Returns `false` if the range is invalid or overlaps an existing mapping.
    ...
    ///
    /// The range must be page-aligned and fully covered by pages created through [`Self::mmap`].
    /// Returns `false` if any page in the range is not currently backed by an mmap area.
